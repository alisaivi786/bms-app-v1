import { addMilliseconds, addMinutes as dateFnsAddMinutes } from 'date-fns';
import { getTimezoneOffset } from 'date-fns-tz';
import { convertSystemDateToZonedDate, convertZonedDateToSystemDate, getDateTimeConfig } from './base';

export const addMinutes = (tzDate: Date, amount: number) => {
	const systemDate = convertZonedDateToSystemDate(tzDate);
	const newDate = dateFnsAddMinutes(systemDate, amount);
